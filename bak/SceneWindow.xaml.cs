...
namespace MyMovie.xaml.Scene
{
    ...
    public partial class SceneWindow : MetroWindow
    {
        ...
        public void Reload(int movie_id, ClipEntity reloadClipEntity)
        {
            movieId = movie_id;
            ...
            else
            {
                ...
                ClipScene.Text = clipEntity.Scene.ToString();
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            if (clipEntity == null)
            {
                ...
                clipEntity.Id = ClipService.GetClipService().CreateClip(clipEntity, 0, 0, movieId, actorEntities);
                MessageBox.Show("保存成功!");
            }
            else {
                clipEntity = EditClipByControl(clipEntity);
                ClipService.GetClipService().UpdateClip(clipEntity, movieId, actorEntities);
                MessageBox.Show("更新成功!");
            }
            (Application.Current as App).SceneWindow.Hide();
            (Application.Current as App).ScenePage.InitPage(movieId);
        }
        ...
        {
            oldClipEntity.Scene = Int32.Parse(ClipScene.Text);
