    ...
    class ScraperService
    {
        ...
        */
        ...
        public void ScraperMovie(Company _Company, Series _Series, Movie _Movie, String webString)
        {
            ...
            else if (webString.Contains("www.clubinfernodungeon.com"))
            {
                if (!File.Exists(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\main.html"))
                {
                    StartChrome();
                    driver.Url = webString;
                    Thread.Sleep(5000);
                    ...
                    ((IJavaScriptExecutor)driver).ExecuteScript("window.scrollBy(0,500)");
                    Thread.Sleep(4000);
                    if (!Directory.Exists(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title)) Directory.CreateDirectory(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title);
                    System.IO.File.WriteAllText(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\main.html", driver.PageSource);
                }
                document = new HtmlWeb().Load(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\main.html");
                ...
                foreach (HtmlNode clipNode in clipNodes)
                {
                    ...
                    if (clipNode.SelectSingleNode(clipNode.XPath + "//a") != null) clipUrl = "https://www.clubinfernodungeon.com" + clipNode.SelectSingleNode(clipNode.XPath + "//a").GetAttributeValue("href", "").Trim();
                    else Console.WriteLine("clipUrl is null");
                    if (clipNode.SelectSingleNode(clipNode.XPath + "/div[1]/div[1]/div[2]/div[1]/div[1]/div[3]/div[1]/div[1]/div[2]/span[1]") != null) clipDate = clipNode.SelectSingleNode(clipNode.XPath + "/div[1]/div[1]/div[2]/div[1]/div[1]/div[3]/div[1]/div[1]/div[2]/span[1]").InnerText;
                    else Console.WriteLine("clipDate is null");
                    ...
                    if (!File.Exists(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\" + (totalNum - resultNum + 1).ToString() + ".html"))
                    {
                        StartChrome();
                        driver.Url = clipUrl;
                        Thread.Sleep(10000);
                        System.IO.File.WriteAllText(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\" + (totalNum - resultNum + 1).ToString() + ".html", driver.PageSource);
                    }
                    HtmlDocument clipDocument = new HtmlWeb().Load(@"D:\VideoTemp\html\" + @"www.clubinfernodungeon.com\" + _Movie.Title + @"\" + (totalNum - resultNum + 1).ToString() + ".html");
                    if (clipDocument.DocumentNode.SelectSingleNode("//div[contains(@class, 'Paragraph')]") != null) clipDescription += clipDocument.DocumentNode.SelectSingleNode("//div[contains(@class, 'Paragraph')]").InnerText.Trim().Replace("&amp;", "&").Replace("&#039;", "'");
                    else Console.WriteLine("clipDescription is null");
                    clips.Add(new string[] { (totalNum - resultNum + 1).ToString(), clipTitle, clipImgUrl, clipUrl, clipDate, clipDescription });
